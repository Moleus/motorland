...
static void rotate(struct Motor* motor, int steps_to_move);

static void step_motor(struct gpiod_line_bulk* lines, const int pin_values[4]);

static void deactivate(struct Motor* motor);
...
struct Motor* setup_motor(uint32_t speed) {
    ...
    ret = gpiod_chip_get_lines(motor->gpio.chip, motor->gpio.pin_offsets, 4, motor->gpio.lines);
    if (ret < 0) {
        perror("Failed to get lines");
        return NULL;
    }

    ret = gpiod_line_request_bulk_output(motor->gpio.lines, CONSUMER_NAME, disabled_values);
    if (ret < 0) {
...
static void rotate(struct Motor* motor, int steps_to_move) {
    ...
    while (steps_left-- > 0) {
        ...
        step_motor(motor->gpio.lines, FULL_SEQUENCE[motor->current_steps & SEQUENCE_MASK]);
...
static void deactivate(struct Motor* motor) {
    int ret = gpiod_line_set_value_bulk(motor->gpio.lines, motor->gpio.disable_values);
    if (ret < 0) {
